...
namespace Installation01.Networking.NetStack.Serialization
{
	public class BitBuffer
	{
		...
		{
			BitBuffer buffer = BufferPool.GetBuffer();
			buffer.Clear();
			return buffer;
		}

		private static readonly byte[] DataBuffer = new byte[4096];
		
		[MethodImpl(256)]
		public static BitBuffer Create(NetworkEvent netEvent)
		{
			BitBuffer buffer = BufferPool.GetBuffer();
			buffer.Clear();
		...
		{
			if (numBits < 0) throw new ArgumentOutOfRangeException("Pushing negative bits");

			if (numBits > 32) throw new ArgumentOutOfRangeException("Pushing too many bits");
		...
		public int ToArray(byte[] data)
		{
			...
			for (int i = 0; i < numChunks; i++)
			{
				...
				if (dataIdx < length) data[dataIdx] = (byte) chunk;

				if (dataIdx + 1 < length) data[dataIdx + 1] = (byte) (chunk >> 8);

				if (dataIdx + 2 < length) data[dataIdx + 2] = (byte) (chunk >> 16);

				if (dataIdx + 3 < length) data[dataIdx + 3] = (byte) (chunk >> 24);
			}
		...
		public void FromArray(byte[] data, int length)
		{
			...
			for (int i = 0; i < numChunks; i++)
			{
				...
				if (dataIdx < length) chunk = (uint) data[dataIdx];

				if (dataIdx + 1 < length) chunk = chunk | ((uint) data[dataIdx + 1] << 8);

				if (dataIdx + 2 < length) chunk = chunk | ((uint) data[dataIdx + 2] << 16);

				if (dataIdx + 3 < length) chunk = chunk | ((uint) data[dataIdx + 3] << 24);
		...
		public BitBuffer AddString(string value)
		{
			...
			if (length > stringLengthMax) length = (uint) stringLengthMax;
		...
		private static byte ToASCII(char character)
		{
			...
			if (value > 127) throw new Exception("Cannot convert to ASCII: " + character);
